/* ============================================================
   LTE Architecture Puzzle — Logic
   Loads data from data/puzzle-data.json
   ============================================================ */
...
function animatePacketOnPath(lineId){
    ...
    packet.setAttribute("class","packet-dot");
...
function resetDropZones() {
    document.querySelectorAll('.drop-zone').forEach(zone => {
        ...
        zone.addEventListener('drop', e => {
            ...
            // Ignore if zone already filled correctly
            if (zone.dataset.filled === 'true') return;

            const draggedSlot = e.dataTransfer.getData('slot');
            const draggedText = e.dataTransfer.getData('text');

            // Both label text AND slot must match (handles duplicate eNodeB)
            const isCorrect =
